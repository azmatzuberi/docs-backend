...
const emailVersionSchema = new mongoose.Schema({
  	first_name: {
		type: String,
		required: true,
		minlength: 1,
...
const Email_Version = mongoose.model('Email_Version', emailVersionSchema);

function validateEmailVersion(email_version) {
  const schema = Joi.object({
	...
	source_email: Joi.string().min(10).max(100).optional()
  });

  return schema.validate(email_version);
}

exports.Email_Version = Email_Version;
exports.validateEmailVersion = validateEmailVersion;
